#!/usr/bin/env python
"""
Nager MCP MCP Server Health Check Script

This script properly connects to the Nager MCP MCP server and checks its health by:
...
def check_mcp_server_health(url: str) -> bool:
    """Check if Nager MCP MCP server is healthy"""
    print(f"🔍 Checking Nager MCP MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected Nager MCP tools are available!")
...
def main():
    parser = argparse.ArgumentParser(description="Check Nager MCP MCP Server Health")
    parser.add_argument("--url", default="http://localhost:8000", 
                       help="Nager MCP MCP server URL (default: http://localhost:8000)")
    args = parser.parse_args()
    
    print(f"🚀 Nager MCP MCP Server Health Check")
    print(f"📍 Server URL: {args.url}")
    print(f"📰 Expected tools: example_tool")
    print("="*50)
    
    if check_mcp_server_health(args.url):
        print("\n✅ Nager MCP MCP Server is healthy and responding!")
        return 0
    else:
        print("\n❌ Nager MCP MCP Server health check failed!")
        return 1
