#!/usr/bin/env python3
"""
Nager MCP MCP Server - FastMCP with D402 Transport Wrapper
...
logger = logging.getLogger('nager-mcp_mcp')
...
logger.info("="*80)
logger.info(f"Nager MCP MCP Server (FastMCP + D402 Wrapper)")
logger.info(f"API: https://date.nager.at")
logger.info(f"Payment: {SERVER_ADDRESS}")
logger.info("="*80)

# Create FastMCP server
mcp = FastMCP("Nager MCP MCP Server", host="0.0.0.0")
...
# - NAGER_MCP_API_KEY: Server's internal API key for payment mode
#
...
def create_app_with_middleware():
    ...
    app.add_middleware(
        ...
        server_name="nager-mcp-mcp-server"  # MCP server ID for tracking
    )
...
if __name__ == "__main__":
    logger.info("="*80)
    logger.info(f"Starting Nager MCP MCP Server")
    logger.info("="*80)
